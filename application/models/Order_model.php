...
class Order_model extends CI_Model
{
    ...
    public function add_order($data_transaction)
    {
        ...
        if (!empty($cart_total)) {
            ...
            if ($this->db->insert('orders', $data)) {
                ...
                $this->_trigger_dropship_fulfillment($order_id);
    ...
    /**
 * ORDER MODEL — DROPSHIPPING INTEGRATION PATCH
 *
 * In your existing application/models/Order_model.php, find the add_order()
 * method. Inside the successful insert block (after add_seller_earnings),
 * ADD the following call:
 *
 *   // Auto-trigger dropship fulfillment for any dropship items
 *   $this->_trigger_dropship_fulfillment($order_id);
 *
 * Then add the private method below to the same class.
 *
 * =============================================================================
 * PASTE THIS METHOD inside your Order_model class
 * =============================================================================
 */

    /**
     * After a successful order, check each order product to see if it is
     * a dropship item, and if so queue it for fulfillment.
     *
     * This marks them as 'queued' — the actual API call happens via
     * Cron_controller::fulfill_dropship_orders() every 5 minutes.
     * This is safer than making a synchronous API call inside the payment
     * confirmation flow (avoids timeouts causing double-charges).
     */
    private function _trigger_dropship_fulfillment($order_id)
    {
        // Load dropshipping model if not loaded
        if (!isset($this->dropshipping_model)) {
        ...
        foreach ($order_products as $op) {
            // Mark as queued so cron picks it up
            $this->db->where('id', $op->id);
            $this->db->update('order_products', [
                'dropship_fulfillment_status' => 'queued',
            ]);
        }
    }
...
/**
 * =============================================================================
 * ALSO: In Order_model::add_order_offline_payment() — same pattern.
 * Only auto-queue when payment is received (not for bank transfer awaiting).
 *
 * Find where payment_status = 'payment_received' is set (Cash on Delivery flow)
 * and add the same call:
 *   $this->_trigger_dropship_fulfillment($order_id);
 * =============================================================================
 *
 * HOW THE QUEUE WORKS:
 *   1. Buyer pays → order saved → _trigger_dropship_fulfillment() marks items as 'queued'
 *   2. Cron (every 5 min) → fulfill_dropship_orders() → finds 'queued' items
 *      but actually the Cron looks for NULL status, so marking as 'queued' is
 *      just a signal. Update the cron query to also pick up 'queued' status:
 *
 *        ->where_in('op.dropship_fulfillment_status', ['queued', null])
 *        ->where('op.dropship_fulfillment_status IS NULL OR op.dropship_fulfillment_status = "queued"', null, false)
 *
 *   3. Cron calls Dropshipping_model::auto_fulfill_order_item() → hits supplier API
 *   4. On success → status = 'submitted', supplier_order_id stored
 *   5. Tracking cron (hourly) → pulls tracking number → emails buyer
 */
